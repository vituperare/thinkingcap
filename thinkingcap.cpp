...
ThinkingCap::ThinkingCap()
{
	...
	this->currentpin = 10;
	this->magneticpin = 9;
	this->transistorpin = 8;
}

ThinkingCap::ThinkingCap(int rpin, int gpin, int bpin, int cpin, int mpin, int tpin)
{
	this->redpin = rpin;
	this->greenpin = gpin;
	this->bluepin = bpin;
	this->currentpin = cpin;
	this->magneticpin = mpin;
	this->transistorpin = tpin;
}

void ThinkingCap::begin()
{
...
void ThinkingCap::tDCS(int rampTime, int stimTime)
{
	...
	for(;;)
	{
		...
		if(val >= 30) break;  //If voltage fully ramped, go on
...
{
	digitalWrite(magneticpin, HIGH); //Activate the tMS circuit
	delay(chargetime);  //Wait for capacitor C1 to charge
	digitalWrite(transistorpin, HIGH); //Tell the transistor to close the second circuit
	digitalWrite(bluepin, HIGH);  //Turn status LED blue
	delayMicroseconds(pulsetime);
	digitalWrite(transistorpin, LOW); //Go away, closed circuit (unnecessary if a thyristor is used instead of a P-type transistor
	digitalWrite(bluepin, LOW);  //Turn status LED off
}

void ThinkingCap::CES(int frequency)
{
	// Haven't put anything here yet
}
